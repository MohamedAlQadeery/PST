		...
		<table class="table table-bordered datatable" id="table-1">
			...
			<tbody>
				...
				<tr>
                    <td>{{++$index}}</td>
                    <td style="width: 10%">
                        <img src="{{$product->product->getImage()}}" class="img img-responsive">
                    </td>
                    <td>{{$product->product->name}}</td>
                    <td class="center">{{$product->product->barcode}}</td>
                    <td class="center">{{$product->price}}</td>
                    <td class="center">{{$product->product->price_to_buy}}</td>
                    <td class="center">{{$product->product->category->name}}</td>
                    ...
                    <td class="center">
                        <a href="{{route('user.products.edit',$product->product->id)}}" class="btn btn-gold">@lang('site.edit')</a>

                        <form action="{{route('user.shopproducts.destroy',$product->product->id)}}" method="post" style="display:inline"
                            onsubmit="return confirm('Are you sure you want to delete this user?');">
                          @csrf()
                          @method('DELETE')
                      <button  class="btn btn-danger"><i class="fa fa-trash"></i>@lang('site.delete')</button>
                      </form>
                         <a href="{{route('user.products.show',$product->product->id)}}" class="btn btn-info">@lang('site.show')</a>
                    </td>
